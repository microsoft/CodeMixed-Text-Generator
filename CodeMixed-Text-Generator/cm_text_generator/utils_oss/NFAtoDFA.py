...
def eClosure(state):
    ...
    finalSet = [singleState for result in [table_nfa.get((s, 'E')) for s in state] for singleState in result]
    ...
    if set(state) == set(finalSet):
        ...
        currentSet = finalSet
        while currentSet != []:
            currentSet = flatten([table_nfa.get((s, 'E')) for s in currentSet])
...
def move(state):
    a = alphabet[:]
    a.remove('E')
    return(list(map(flatten, [[table_nfa.get((s, i)) for s in state] for i in a])))
...
def printTransitionTable(states, initial_state, final_states, table):
    ...
    for i in states:
        print((str(states.index(i) + 1)), end=' ')
        for j in range(0, len(alphabet) - 1):
            transition = table.get((str(i), alphabet[j]))
            if transition == []:
                print(("\t" + "{}"), end=' ')
            else:
                print(("\t" + "{" + str(table.get((str(i), alphabet[j]))) + "}"), end=' ')
        print()
    print()
    ...
    for state in states_dfa:
        ...
        for i in range(0, len(alphabet) - 1):
            if moves[i] != []:
                    print((listToSetString(state) + " --- " + alphabet[i] + " --> " + listToSetString(moves[i])))
                    e = eClosure(moves[i])
                    print(("E-closure" + listToSetString(moves[i]) + " = " + listToSetString(e) + " ="), end=' ')
                    print((states_dfa.index(e) + 1))
        createEntry(state)
