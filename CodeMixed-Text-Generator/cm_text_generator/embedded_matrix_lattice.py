...
def makeEmLattice(node, grammar, model, ln_theory):
  global linguistic_theory
  ...
  if node.token != 'XXXXX':
    ...
    node.doof.addTransition(a, node.token+"_"+node.language[0], b, [("push", node.label + '_' + node.language[0]), ("match"), ("pop")])

    c = node.counterpart.doof.addState()
    d = node.counterpart.doof.addState()
    node.counterpart.doof.addTransition(c, node.counterpart.token+"_"+node.counterpart.language[0], d, [("push", node.counterpart.label + '_' + node.counterpart.language[0]), ("match"), ("pop")])
    ...
    for child in node.children:
      makeEmLattice(child, grammar, model, linguistic_theory)
      node.doof.mergeDfaSeries(child.doof, linguistic_theory)
      cond = cond and child.fullySwitchable
      
    for child in node.counterpart.children:
      node.counterpart.doof.mergeDfaSeries(child.doof, linguistic_theory)
...
def mergeParallelViaNfa(dfa1, dfa2):
  ...
  dfa1.mergeDfa(dfa2, linguistic_theory)
  ...
  if len(mergedDfa.finalStates) > 1:
    mergedDfa.mergeStates(mergedDfa.finalStates[0], mergedDfa.finalStates[1:], linguistic_theory)
